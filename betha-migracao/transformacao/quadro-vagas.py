        ...
        if len(registros) > 0:
            for item in registros:
                dado = {
                    ...
                    'hash_chave_dsk': encurtar(sistema, tipo_registro, item['anoLetivo']['id'],item['estabelecimento']['id'],item['etapaMatriz']['id'],item['turno']),
                    'descricao_tipo_registro': f'Cadastro de {tipo_registro}',
                    'id_desktop': id_desktop if id_desktop is not None else None,
                    'json': dumps(item),
                    'i_chave_dsk1': item['anoLetivo']['id'],
                    'i_chave_dsk2': item['estabelecimento']['id'],
                    'i_chave_dsk3': item['etapaMatriz']['id'],
                    'i_chave_dsk4': item['turno']
                }
...
def enviar(registros):
    ...
    try:
        ...
        for item in registros:
            ...
            dado = {
                'idIntegracao': encurtar(sistema, tipo_registro, item['anoletivo'],item['estabelecimento'],item['etapamatriz'],item['turno']),
                'conteudo': {
                    'estabelecimento': {
                    ...
                    'turno': item['turno'],
                    'qtdVagas': int(item['qtdvagas']),
                    'qtdVagasReservadas': item['qtdvagasreservadas']
                }
            }
