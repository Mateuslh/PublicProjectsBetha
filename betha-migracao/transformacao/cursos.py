        ...
        for item in registros:
            ...
            dado = {
                ...
                'curso': {
                    ...
                    'nivelModalidade': item['modalidade'],
                    'organizacao': {
                        'id': item['organizacao']
                    },
            ...
            if 'nivel' in item and item['nivel'] is not None:
                dado['curso'].update({
                    'nivelEscolar': item['nivel']
                })

            # print(f'@ Dado(s) gerado(s) ({contador}): ', dado)
            lista_dado.append(dado)
            lista_controle.append(formatar([dado['curso']],item['id_desktop'])[0])
    except Exception as e:
