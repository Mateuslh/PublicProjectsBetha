        ...
        if len(registros) > 0:
            for item in registros:
                dado = {
                    ...
                    'hash_chave_dsk': encurtar(sistema, tipo_registro, item['identificacao'].upper(),id_desktop),
                    'descricao_tipo_registro': f'Cadastro de {tipo_registro}',
                    'id_desktop': id_desktop if id_desktop is not None else None,
                    'json': dumps(item),
                    'i_chave_dsk1': item['identificacao'].upper(),
                    'i_chave_dsk2': id_desktop
                }
...
def enviar(registros):
    ...
    try:
        ...
        for item in registros:
            ...
            dado = {
                'idIntegracao': encurtar(sistema, tipo_registro, item['identificacao'].upper(),item['id_desktop']),
                'conteudo': {
                    'identificacao': item['identificacao'],
                    'quantidadeDependencias': item['quantidadedependencias'],
                    'classificacao': item['classificacao'],
                    'corClassificacao': item['corclassificacao'],
                    'anoIdadeMinima': item['anoidademinima'],
                    'anoIdadeMaxima': item['anoidademaxima'],
                    'mesIdadeMinima': item['mesidademinima'],
                    'mesIdadeMaxima': item['mesidademaxima'],
                    'ordem': item['ordem'],
                    'etapaInep': {
                        'id':item['inep']
                    },
                    ...
                    'diaCompletarIdadeMinima': item['diacompletaridademinima'],
                    'mesCompletarIdadeMinima': item['mescompletaridademinima'],
                    'matriz': {
                        'id': item['matriz']
                    }
                }
