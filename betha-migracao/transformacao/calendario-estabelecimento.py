        ...
        if len(registros) > 0:
            for item in registros:
                dado = {
                    ...
                    'hash_chave_dsk': encurtar(sistema, tipo_registro,item['estabelecimento']['id'],item['anoLetivo']['id']),
                    'descricao_tipo_registro': f'Cadastro de {tipo_registro}',
                    'id_desktop': id_desktop if id_desktop is not None else None,
                    'json': dumps(item),
                    'i_chave_dsk1': item['estabelecimento']['id'],
...
def enviar(registros):
    ...
    try:
        ...
        for item in registros:
            ...
            dado = {
                'idIntegracao': encurtar(sistema, tipo_registro,item['estabelecimento'],item['anoletivo']),
                'conteudo': {
                    'anoLetivo': {
                        'id': item['anoletivo']
                    },
                    ...
                    'calendarioSecretaria': {
                        'id': item['calendariosecretaria']
                    },
                    'calendario':{
                        'anoLetivo': {
                            'id': item['anoletivo']
