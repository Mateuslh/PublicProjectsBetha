    ...
    try:
        ...
        for item in registros:
            ...
            if 'cpf' in item and item['cpf'] is not None:
                dado['responsavel']['pessoa'].update({'cpf': item['cpf']})
            if 'pis' in item and item['pis'] is not None:
                dado['responsavel']['pessoa'].update({'pis': item['pis']})
            if 'rg' in item and item['rg'] is not None:
                dado['responsavel']['pessoa'].update({'rg': item['rg']})
            if 'numerotermo' in item and item['numerotermo'] is not None:
                dado['responsavel']['pessoa'].update({'numeroTermo': item['numerotermo']})
            if 'telefones' in item and item['telefones'] is not None:
                dado['responsavel']['pessoa'].update({
                    'telefones': []
                })
                ...
                if len(lista) > 0:
                    for listacampo in lista:
                        ...
                        dado['responsavel']['pessoa']['telefones'].append({
                            'descricao': campo[0],
                            'tipoNumero': campo[1],
                            'tipo': campo[2],
                            'telefone': campo[3],
            ...
            if 'emails' in item and item['emails'] is not None:
                ...
                if len(lista) > 0:
                    for listacampo in lista:
                        campo = listacampo.split('%|%')
                        dado['responsavel']['pessoa']['emails'].append({
                            'descricao': campo[0],
                            'email': campo[1],
                            'tipo': campo[2]
                        })
            ...
            lista_controle.append(formatar([dado['responsavel']], item['id_desktop'])[0])
    except Exception as e:
        print(f'\n* Erro ao executar função "enviar" {e}')
    finally:
