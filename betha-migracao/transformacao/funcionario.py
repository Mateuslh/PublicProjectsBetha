        ...
        if len(registros) > 0:
            for item in registros:
                dado = {
                    ...
                    'i_chave_dsk2': item['pessoa']['fisica']['cpf']
                }
...
def enviar(registros):
    ...
    try:
        ...
        for item in registros:
            ...
            dado = {
                'idIntegracao': encurtar(sistema, tipo_registro, item['nome'].upper(), item['cpf']),
                'conteudo': {
                    'pessoa': {
                        'nome': item['nome'],
                        'fisica': {
            ...
            if 'datanascimento' in item and item['datanascimento'] is not None:
                dado['conteudo']['pessoa']['fisica'].update({'dataNascimento': item['datanascimento']})
            if 'sexo' in item and item['sexo'] is not None:
                dado['conteudo']['pessoa']['fisica'].update({'sexo': item['sexo']})
            if 'rg' in item and item['rg'] is not None:
                dado['conteudo']['pessoa']['fisica'].update({'rg': item['rg']})
            if 'pis' in item and item['pis'] is not None:
                dado['conteudo']['pessoa']['fisica'].update({'pis': item['pis']})
            if 'numerotermo' in item and item['numerotermo'] is not None:
                dado['conteudo']['pessoa']['fisica'].update({'numeroTermo': item['numerotermo']})
            if 'telefones' in item and item['telefones'] is not None:
                dado['conteudo']['pessoa'].update({
                    'telefones': []
                })
                ...
                if len(lista) > 0:
                    for listacampo in lista:
                        ...
                        dado['conteudo']['pessoa']['telefones'].append({
                            'descricao': campo[0],
                            'tipoNumero': campo[1],
                            'tipo': campo[2],
                            'telefone': campo[3],
            ...
            if 'emails' in item and item['emails'] is not None:
                ...
                if len(lista) > 0:
                    for listacampo in lista:
                        campo = listacampo.split('%|%')
                        dado['conteudo']['pessoa']['emails'].append({
                            'descricao': campo[0],
                            'email': campo[1],
                            'tipo': campo[2]
                        })
